...
import { getDailyCashAction } from "@/actions/daily-cash";
...
export function CashReport({ cashId }: CashReportProps) {
  const [cashData, setCashData] = useState<any>(null);
  const [isLoading, setIsLoading] = useState(true);

  const { execute: executeGetCash } = useAction(getDailyCashAction, {
    onSuccess: ({ data }) => {
      setCashData(data);
      setIsLoading(false);
    },
  ...
  useEffect(() => {
    // Buscar dados do caixa específico
    executeGetCash({});
  }, [cashId]);
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      // Horários
      doc.text(
        `Abertura: ${format(cashData.openingTime, "HH:mm", { locale: ptBR })}`,
        margin,
        yPosition,
      );
      yPosition += 7;
      ...
      if (cashData.closingTime) {
        doc.text(
          `Fechamento: ${format(cashData.closingTime, "HH:mm", { locale: ptBR })}`,
          margin,
          yPosition,
        );
        yPosition += 7;
  ...
  if (!cashData) {
    return (
      <div className="py-8 text-center">
        <p className="text-muted-foreground">Dados do caixa não encontrados.</p>
      </div>
    );
  }
