...
export function FinancialSummary({
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.text("RESUMO FINANCEIRO DETALHADO", pageWidth / 2, yPosition, {
        align: "center",
      });
      yPosition += 15;
      ...
      if (periodStart && periodEnd) {
        doc.text(
          `Período: ${format(periodStart, "dd/MM/yyyy")} - ${format(periodEnd, "dd/MM/yyyy")}`,
          pageWidth / 2,
          yPosition,
          { align: "center" },
        );
      ...
      doc.text(
        `Total de Receitas: ${formatCurrencyInCents(totalRevenue)} (${revenues.length} transações)`,
        margin,
        yPosition,
      );
      yPosition += 7;
      ...
      doc.text(
        `Total de Despesas: ${formatCurrencyInCents(totalExpenses)} (${expenses.length} transações)`,
        margin,
        yPosition,
      );
      yPosition += 7;
      ...
      doc.text(
        `LUCRO LÍQUIDO: ${formatCurrencyInCents(netProfit)}`,
        margin,
        yPosition,
      );
      yPosition += 5;
      doc.text(
        `Status: ${netProfit >= 0 ? "RESULTADO POSITIVO" : "RESULTADO NEGATIVO"}`,
        margin,
        yPosition,
      );
      yPosition += 20;
      ...
      if (filteredTransactions.length > 0) {
        ...
        doc.text("Descrição", margin + 30, yPosition);
        doc.text("Tipo", margin + 100, yPosition);
        ...
        // Mostrar até 30 transações
        const transactionsToShow = filteredTransactions.slice(0, 30);
        ...
        transactionsToShow.forEach((transaction) => {
          ...
          doc.setTextColor(0);
          doc.text(
            format(new Date(transaction.createdAt), "dd/MM"),
            margin,
            yPosition,
          );
          ...
          doc.text(typeLabel, margin + 100, yPosition);
        ...
        if (filteredTransactions.length > 30) {
          ...
          doc.setTextColor(0);
          doc.text(
            `... e mais ${filteredTransactions.length - 30} transações`,
            margin,
            yPosition,
          );
        }
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">
            Visualização detalhada de todas as transações financeiras.
          </p>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {formatCurrencyInCents(totalExpenses)}
            </div>
            <p className="text-muted-foreground text-xs">
